...
def main():
    if len(sys.argv) != 5:
        print("Usage: weekly-sqlite.py <outpaths.json> <missing-packages.txt> <attic-db> <cache-name>")
        sys.exit(1)
    ...
    missing = [name for name, path in sorted(outpaths.items()) if store_hash(path) not in cached]
    ...
    if outpaths and cached_count == 0:
        print("warning: no evaluated store paths are present in Attic; changing R_NIXPKGS_DATE can change every output hash")
    if missing:
